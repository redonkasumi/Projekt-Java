...
public class StudentDatabase {
	private Student[] base;   // the collection of records
	  ...
	  public StudentDatabase(int initial_student_size)
	  { if ( initial_student_size > 0 )
	         { base = new Student[initial_student_size]; }
	    else { base = new Student[1]; }
	  }
	  ...
	  { int result = NOT_FOUND;
	    ...
	    while ( !found  &&  i != base.length )
	          { if ( base[i] != null  &&  base[i]. getKeyOfStudent() .equals(k) )
	                 { found = true;
	                   result = i;
	                 }
	            else { i = i + 1; }
	  ...
	  public Student findStudent(Key k)
	  { Student answer = null;
	  ...
	  public boolean insertStudent(Student r)
	  { boolean success = false;
	    if ( findLocation(r.getKeyOfStudent()) == NOT_FOUND )  // r  not already in  base?
	       { // find an empty element in  base  for insertion of  r:
	         ...
	         if ( found_empty_place )
	              { base[i] = r; }
	                ...
	                Student[] temp = new Student[base.length * 2];
	  ...
	  public boolean deleteStudent(Key k)
